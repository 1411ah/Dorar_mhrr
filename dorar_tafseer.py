...
def get_tip_text(tip):
    """استخراج نص الحاشية من أي مكان تخزّنه"""
...
def extract_content(html):
    ...
    for art in articles:
        ...
        # ── 1. استبدل كل span.tip بعلامة \x01N\x01 (recursive) ──
        tips_map = {}
...
def renum(text, fns, global_fn_ref):
    """
    إعادة ترقيم الحواشي من أرقام محلية إلى أرقام عالمية.
    يستخدم placeholder مؤقت \x02N\x02 لتفادي التداخل بين الأرقام.
    global_fn_ref: قائمة بعنصر واحد [int] تُعدَّل في المكان.
    """
    if not fns:
        return text, []

    # استخرج الخريطة: رقم محلي → رقم عالمي
    local_map = {}
    for fn in fns:
        m = re.match(r'\[\^(\d+)\]:', fn)
        if m:
    ...
    # المرحلة الأولى: استبدل [^N] بـ \x02N\x02 (placeholder)
    ...
    # المرحلة الثانية: استبدل \x02N\x02 بالرقم العالمي النهائي
    for loc, gbl in local_map.items():
        text = text.replace(f'\x02{loc}\x02', f'[^{gbl}]')

    # أعد بناء تعريفات الحواشي بالأرقام العالمية
    new_fns = []
    for fn in fns:
...
def save_markdown(surah_title, surah_num, intro, sections):
    ...
    all_footnotes  = []
    global_fn_ref  = [1]   # قائمة بعنصر واحد حتى تُعدَّل داخل renum
